...
export const PRICES: Price[] = [
  ...
  },
  {
    ...
    now: '$87', list: '$145', activation: '無', total: '$87', code: 'PFTW',
    link: 'https://tradeify.co/?ref=ZIPOK2X5',
    note: '1 天可通關，但出金要墊到 $53,000',
  },
  {
    id: 'tradeify', name: 'Tradeify · Select', logo: '/logos/tradeify.png', model: '一次性',
    now: '$99', list: '$165', activation: '無', total: '$99', code: 'PFTW',
    link: 'https://tradeify.co/?ref=ZIPOK2X5',
...
export const OFFERS: Offer[] = [
  ...
  { firm: 'Tradeify — Select 50K', old: '原價 $165', now: '$99', note: '6 折 · 通關後選 Flex（無緩衝）或 Daily（每日領）', deadline: '折扣碼 PFTW · 前 5 次 6 折，之後 7 折', code: 'PFTW', link: 'firms/tradeify' },
  { firm: 'Tradeify — Growth 50K', old: '原價 $145', now: '$87', note: '6 折 · 1 天可通關 · 出金需墊到 $53,000', deadline: '折扣碼 PFTW · 前 5 次 6 折，之後 7 折', code: 'PFTW', link: 'firms/tradeify' },
...
export const PRODUCTS: Record<string, FirmProducts> = {
  lucid: {
    ...
    exchanges: 'CME・CBOT・NYMEX・COMEX（官方核准共 34 檔 CME 集團期貨）',
    allow: [
      '股指：ES NQ RTY NKD YM（含微型 MES MNQ M2K MYM）',
      '外匯：6A 6B 6C 6E 6J 6S 6N',
      '能源：CL NG QM QG（含微型 MCL）',
      '金屬：GC SI HG PL（含微型金 MGC・微型銀 SIL）',
      '農產品：ZC ZS ZW ZM ZL・HE LE',
    ],
    deny: [
      '比特幣／加密期貨',
      'VIX',
    ...
    note: '官方核准 34 檔：股指/外匯/能源/金屬/農產品都有，但不做加密、VIX、美債利率、軟性商品。另禁高頻、禁 5 秒內微剝頭皮佔比過半、禁對沖。（官方幫助中心 2026/2/9 ✅）',
  },
  apex: {
    platforms: 'Tradovate / NinjaTrader / Rithmic 等',
    exchanges: 'CME・CBOT・NYMEX・COMEX（CME 集團期貨）',
    allow: [
      '股指：ES NQ YM RTY（含微型 MES MNQ MYM M2K）',
      '能源：CL NG（含微型 MCL）',
      '利率：ZT ZB TN 等美債期貨',
      '農產品：ZC ZS ZW ZL・HE LE',
      '外匯：6E 6B 6J 6A 6C 6S',
      '微型加密：MBT（微比特幣）・MET（微以太）',
      '金屬（2026/5 恢復）：GC SI MGC HG PL PA',
    ],
    deny: ['股票・選擇權・現貨外匯・CFD', '小帳號因跳動值大，全額 BTC 期貨通常不可'],
    note: '商品很齊——連微型加密與美債利率都有，金屬 2026/5 起全帳號恢復。⚠️ Apex 官網封鎖機房 IP，此清單以評測／官方摘要整理、未逐項對官方，實際以你 Apex 後台商品清單為準。',
  },
  topstep: {
    platforms: 'TopstepX™（內建 TradingView）',
    exchanges: 'CME・CBOT・COMEX・NYMEX（僅期貨；禁 Forex、禁隔夜）',
    allow: [
      '股指：ES NQ RTY NKD YM（含微型 MES MNQ M2K MYM）',
      '微型加密：MBT（微比特幣）・MET（微以太）',
      '外匯：6A 6B 6C 6E 6J 6S E7 6N 6M（含微型 M6E M6A M6B）',
      '利率：ZT ZF ZN TN ZB UB',
      '能源：CL NG QM QG RB HO（含微型 MCL MNG）',
      '金屬：GC SI HG PL（含微型 MGC SIL MHG）',
      '農產品：ZC ZW ZS ZM ZL・HE LE',
    ],
    deny: [
      '股票・選擇權・現貨外匯・現貨加密・CFD',
      '禁隔夜／波段：每日 3:10 PM CT 前一律平倉',
      '標 * 的商品（各微型、微加密）另有交易行為限制，見官方 Prohibited Conduct',
    ],
    note: '純期貨當沖，商品很齊——連微型比特幣/以太、金屬都能做；但禁 Forex、禁隔夜，收盤前要空手。（官方幫助中心 ✅）',
  },
  tradeify: {
    platforms: 'Tradovate / NinjaTrader / Rithmic / TradingView 等',
    exchanges: 'CME・COMEX・NYMEX・CBOT ＋ 🇪🇺 EUREX（歐洲，需另訂閱數據）',
    allow: [
      '股指（迷你）：ES NQ YM RTY EMD NKD',
      '微型股指：MES MNQ MYM M2K',
      '🇪🇺 歐洲 EUREX 指數：DAX(FDAX)・迷你DAX(FDXM)・微型DAX(FDXS)・Euro Stoxx 50(FESX)・微型(FSXE)・STOXX 600(FXXP)・VSTOXX(FVS)',
      '🇪🇺 歐洲 EUREX 債券：Euro-Bund(FGBL)・Bobl(FGBM)・Schatz(FGBS)・Buxl(FGBX)',
      '外匯：6A 6B 6C 6E 6J 6S（含微型 M6A M6E）',
      '能源：CL NG QM QG MCL',
      '金屬：GC SI HG PL PA（含 E-mini 金 QO・微型金 MGC）',
      '農產品：ZC ZS ZW ZM ZL LE HE GF',
    ],
    deny: [
      '加密貨幣衍生品（含比特幣期貨，一律不支援）',
      '股票・選擇權・現貨外匯',
      '⚠️ 沒有微型白銀（SIL 不支援；唯一微型金屬是 MGC）',
    ],
    note: '唯一提供歐洲 EUREX（DAX、Euro Stoxx、歐債）的一家，想做歐盤選它——EUREX 用歐元計價、美元結算。不支援任何加密。（依官方 2026/5/20 清單）',
  },
  tradeday: {
    platforms: 'Tradovate / NinjaTrader / Rithmic 等',
    exchanges: 'CME・CBOT・COMEX・NYMEX（僅 CME 集團期貨）',
    allow: [
      '股指：ES NQ RTY NKD YM（含微型 MES MNQ M2K MYM）',
      '外匯：6A 6B 6C 6E 6J 6S 6N（含微型 M6E）',
      '利率：ZT ZF ZN ZB UB TN（完整美債期貨）',
      '能源：CL NG QM QG（含微型 MCL）',
      '農產品：ZC ZW ZS ZM ZL・HE LE',
    ],
    deny: [
      '加密貨幣・股票・選擇權・現貨外匯・CFD',
      '⚠️ 金屬暫時停用（2026/2/11 起・全帳號）：GC SI HG PL 目前不可交易',
      '微型金 MGC／微型銀 SIL 部位上限 4 口',
      '不能在價格限制 2% 範圍內交易',
    ],
    note: '主力是股指、外匯、完整美債利率期貨；不做加密。⚠️ 2026/2 起金屬（GC/SI/HG/PL）暫停、微型金銀限 4 口。（官方幫助中心 2026/2/11 ✅）',
  },
};
